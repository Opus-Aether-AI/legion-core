...
| `legion-improve {run\|queue} --repo . [--mode off\|dry-run\|draft]` | Review-only typed improvement engine. `off` is the default; `draft` freezes an exact base in an isolated worktree, uses a native Markdown-only mutator, repeats paired gates, requires `legion-delegate review`, and creates at most one draft PR. Never merges, deploys, or edits the operator checkout. |
...
| `legion-self-learn run --apply-memory` | Mine spans, review verdicts, trigger evals, benchmark misses, manual bug records, and routing optimizer advice; attach failures to catalog entities (plugin/skill/command/agent/hook/MCP) and write durable memory/proposals. `--apply-source` is compatibility-only and directs callers to `legion-improve --mode dry-run`. |
