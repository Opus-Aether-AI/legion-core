...
- **"Learn from the last few sessions"** → `legion-session-learn --repo . --query <term> --record`, then `legion-self-learn run --apply-memory` so the findings become active hints. Use `--show-evidence` only when a human needs redacted excerpts. Daily refresh does the bounded broad no-query scan automatically.
