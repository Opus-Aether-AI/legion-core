...
| `legion-self-learn run --apply-memory` | Mine spans, review verdicts, trigger evals, benchmark misses, manual bug records, and routing optimizer advice; attach failures to catalog entities and promote bounded typed hints under a project lock. Cross-project laws retain global scope; retirement and capacity rejection are explicit. `--apply-source` is compatibility-only and directs callers to `legion-improve --mode dry-run`. |
