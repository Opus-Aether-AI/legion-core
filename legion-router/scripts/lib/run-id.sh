...
legion_new_run_id() {
  local stamp entropy="" date_cmd="" od_cmd="" tr_cmd="" uuid_cmd="" uuid=""
  date_cmd="$(command -v date 2>/dev/null || true)"
  od_cmd="$(command -v od 2>/dev/null || true)"
  tr_cmd="$(command -v tr 2>/dev/null || true)"
  uuid_cmd="$(command -v uuidgen 2>/dev/null || true)"
  [[ -n "$date_cmd" ]] || { [[ ! -x /bin/date ]] || date_cmd=/bin/date; }
  [[ -n "$date_cmd" ]] || { [[ ! -x /usr/bin/date ]] || date_cmd=/usr/bin/date; }
  ...
  if [[ -z "$date_cmd" ]]; then
    printf 'legion: run-id timestamps are unavailable (need date)\n' >&2
    return 1
  fi
  stamp="$("$date_cmd" -u +%Y%m%d-%H%M%S)" || return 1
