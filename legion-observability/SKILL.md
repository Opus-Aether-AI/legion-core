...
| `legion-share [next] [--target T]` | Measure the **codex-vs-Opus work split** (by runs + tokens, per model) vs the target (default 0.5). `legion-share next` → `codex`/`opus`: who should do the next task to converge. Requires Opus to log its own work via `legion-trace emit --executor opus …` so there's a denominator. |
