...
class Broker:
    ...
    def _copy_auth_file(self, source: Path, destination: Path) -> None:
        try:
    ...
    def _prepare_private_runtime(self) -> None:
        ...
        for name in ("tmp", "cache", "state", "xdg-config", "xdg-cache", "xdg-state", "xdg-data"):
    ...
    def _copy_telemetry(self) -> None:
        if self.telemetry_dir is None:
            return
        source_dir = self.broker_root / "runtime" / "state" / "spans"
        ...
        for source in sorted(source_dir.iterdir()):
            if not SAFE_TELEMETRY_NAME.fullmatch(source.name):
                continue
            source_stat = source.lstat()
            if not stat.S_ISREG(source_stat.st_mode):
                continue
            lines: list[bytes] = []
            destination = self.telemetry_dir / source.name
            with source.open("rb") as stream:
                for raw_line in stream:
                    copied += len(raw_line)
                    if copied > MAX_TELEMETRY_BYTES:
                        raise ValueError("nested handoff telemetry exceeds 4 MiB")
                    payload = json.loads(raw_line)
                    if (
                        not isinstance(payload, dict)
                        or payload.get("schema") != "legion.span.v1"
                        or payload.get("parent_id") != self.expected_parent
                    ):
                        raise ValueError("nested handoff emitted invalid telemetry attribution")
                    encoded = json.dumps(payload, separators=(",", ":")).encode() + b"\n"
                    lines.append(encoded)
            pending[destination.name] = lines

        for name, lines in pending.items():
            destination = self.telemetry_dir / name
